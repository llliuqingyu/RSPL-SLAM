...
//已知左图xy，右图x。根据左右图x算出深度。再算出3d的xy，再乘深度恢复3d点
bool Camera::BackProjectStereo(const Eigen::Vector3d& keypoint, Eigen::Vector3d& output){
  BackProjectMono(keypoint.head(2), output);//keypoint的前两维是左图xy，第三维是右图的x
  double d = _bf / (keypoint(0) - keypoint(2));//深度
  output = output * d;//xy恢复深度
  return true;
}