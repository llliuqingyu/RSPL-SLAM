...
struct InputData{
  size_t index;//size_t 的主要目的是提供一种与平台无关的方式来表示大小和长度，取决于编译器。64位操作系统中，2^64 - 1
  double time;
  cv::Mat image_left;
  cv::Mat image_right;
  cv::Mat image_left_rcf;
  cv::Mat image_right_rcf;
  ...
  InputData& operator =(InputData& other){
        ...
        image_left_rcf = other.image_left_rcf.clone();
		image_right_rcf = other.image_right_rcf.clone();
		return *this;
	}
};
typedef std::shared_ptr<InputData> InputDataPtr;//图片id、时间戳、左右图片、深拷贝
